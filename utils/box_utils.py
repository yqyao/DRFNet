...
import math
import numpy as np
from utils.nms.cpu_nms import get_iou_weights
...
def soft_match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx, loc_data):
    """Match each prior box with the ground truth box of the highest jaccard
    overlap, encode the bounding boxes, then return the matched indices
    corresponding to both confidence and location preds.
    Args:
    ...
    """
    # jaccard index
    decoded_boxes = decode(loc_data[idx], priors, variances)    
    overlaps = jaccard(
        truths,
        decoded_boxes
    )
