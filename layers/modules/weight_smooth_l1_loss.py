...
class WeightSmoothL1Loss(nn.Module):
    ...
    def forward(self, inputs, targets, weights):
        ...
        # C = conf_inputs.size(1)
        # P = F.softmax(conf_inputs)

        # class_mask = conf_inputs.data.new(N, C).fill_(0)
        # class_mask = Variable(class_mask)
        # ids = conf_targets.view(-1, 1)
        # class_mask.scatter_(1, ids.data, 1.)

        if inputs.is_cuda and not weights.is_cuda:
            weights = weights.cuda()

        # weights = (1 - p) * iou(weight)
        # weights = (1. - probs) * weights.view(-1, 1)
