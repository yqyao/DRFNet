...
class COCODetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.target_transform is not None:
            boxes_t, labels_t = self.target_transform(target, width, height)

        if self.transform is not None:

            img, boxes_t, labels_t = self.transform(img, boxes_t, labels_t)
            if len(boxes_t) == 0 or len(labels_t) == 0:
                target = np.zeros((1,5))
            else:
                target = np.hstack((boxes_t, np.expand_dims(labels_t, axis=1)))
