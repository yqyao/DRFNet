...
parser = argparse.ArgumentParser(
    description='Dense Receptive Field Net Training')
...
criterion = RefineMultiBoxLoss(num_classes, 0.5, True, 0, True, 3, 0.5, False, 0.01, args.cuda)
...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration % 100 == 0:
            debug = True
        arm_loss_l, arm_loss_c = arm_criterion(out, targets, debug=debug)
        loss_l, loss_c = criterion(out, targets, use_arm=True, filter_object=True, debug=debug)

        loss = arm_loss_l + arm_loss_c + loss_l + loss_c
        loss.backward()
        ...
        if iteration % 10 == 0:
            print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % epoch_size) + '/' + repr(epoch_size)
                  + '|| Totel iter ' +
                  repr(iteration) + 
                  ' || arm_L: %.4f arm_C: %.4f||' % (arm_loss_l.data[0],arm_loss_c.data[0]) + 
                  ' L: %.4f C: %.4f||' % (loss_l.data[0],loss_c.data[0]) + 
                  ' loss: %.4f||' % (loss.data[0]) +
                'iteration time: %.4f sec. ||' % (load_t1 - load_t0) + 'LR: %.8f' % (lr))

    torch.save(net.state_dict(), args.save_folder +
               '0419_Final_bgr_' + args.version +'_'+channel_size+'_' + args.dataset+"_"+args.size+ '.pth')
